...
- **A2A Agent Management** — Register A2A (Agent-to-Agent) protocol agents by base URL with automatic Agent Card fetching. View structured Agent Card details including capabilities, authentication schemes, input/output modes, and skills. OAuth2 authentication configuration with test connection. Per-persona access control with all_skills or selected_skills granularity
...
- **Persona Navigation:** Sidebar with Platform Catalog, Agents, Memory, Security Admin, MCP Servers, A2A Agents, Tagging, Settings. All navigation items fully active
